...
void loop() {
  ...
  if (menu == true) {
    ...
    if (gameOver) { //game over animation
      ...
      moveInterval = 300 - ((millis()-speedModifier) / 1000);
    if(moveInterval<=100)moveInterval=100;
      if (millis() - lastMoved > moveInterval) {
        // game logic
        directie = command;
        lastMoved = millis();
...
void checkNext() {
  ...
  if (xNext == foodx && yNext == foody) { //daca a gasit mancare
    ...
    score = score + 1;
    score = score + ((foodTimer+5000-millis())/100);
    lcd.setCursor(3, 1);
    lcd.print("Score : ");
    lcd.print(score);
